...
import iconFinish from '../../assets/imgs/SignUp/icon-finish-signUp.png';
...
function EditUserModal({ setOpenUserEditModal, openUserEditModal }) {
  ...
  async function handleSendForm() {
    ...
    const user = {
      name: valueInput.name.trim(),
      email: valueInput.email.trim(),
      password: valueInput.password2.trim(),
      cpf: valueInput.cpf.trim(),
      phone: valueInput.phone.trim(),
    };
  ...
  function handleClickShowPassword(input) {
    input
      ? setShowPassword((prevValue) => ({ ...prevValue, input2: !showPassword.input2 }))
  ...
  return (
    ...
    >
      {!finishUpdate ? (
        ...
        <div
          className="finish-signUp"
          onClick={() => setOpenUserEditModal(false)}
        >
          <img src={iconFinish} alt="Fim do cadastro" />
